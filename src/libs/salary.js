...
import { formatterValue } from "../helpers/numberFormat";
...
function calculateSalaryFrom(fullSalary) {
  const baseINSS = parseInt(fullSalary) || 0;

  const discountINSS = calculateDiscountINSS(baseINSS) || 0;
  const discountINSSPercentage =
    parseFloat(((discountINSS / baseINSS) * 100).toFixed(2)) || 0;

  const baseIRPF = baseINSS - discountINSS || 0;
  const discountIRPF = calculateDiscountIRPF(baseIRPF) || 0;
  const discountIRPFPercentage =
    parseFloat(((discountIRPF / baseINSS) * 100).toFixed(2)) || 0;

  const netSalary = baseINSS - discountINSS - discountIRPF || 0;
  const netSalaryPercentage =
    parseFloat(((netSalary / baseINSS) * 100).toFixed(2)) || 0;
